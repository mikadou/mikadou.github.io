...
async function trainRL() {
  ...
  statusEl.textContent = `PPO training complete · curriculum reached n=${reachedN}. Running benchmark…`;
  await runBenchmark();
}
...
async function runBenchmark() {
  ...
  for (const n of benchmarkLengths()) {
    statusEl.textContent = `Benchmarking PPO GNN at n=${n}…`;
    let policySolved = 0;
    let saSolved = 0;
    const policyMoves = [];
    for (let t = 0; t < trials; t++) {
  ...
  $('metricLength').textContent = `n=${n} · domain 0…${DOMAIN_MAX} · PPO terminal reward · curriculum reached n=${reachedN}`;
  statusEl.textContent = `Done. PPO uses factorized variable/value actions, a learned critic, and at most ${MOVE_MULTIPLIER}n moves with early success termination.`;
...
if (window.tf) {
  tf.ready().then(() => {
    statusEl.textContent = `TensorFlow.js ready · backend: ${tf.getBackend()}. Train the PPO actor-critic GNN curriculum to begin.`;
  });
} else {

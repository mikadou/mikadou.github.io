...
const VALUE_NEGATIVES = 40;
const PROPOSAL_SAMPLES = 28;
const RANDOM_CANDIDATES = 14;
const VARIABLE_TEMPERATURE = 0.30;
const VALUE_TEMPERATURE = 0.45;
const VARIABLE_THRESHOLD = 0.5;
const VARIABLE_MARGIN = 2.0;
const PROPOSAL_LOSS_WEIGHT = 0.30;
const VALUE_SCORE_LOSS_WEIGHT = 0.45;
...
function installDiagnosticsUI() {
  ...
  section.innerHTML = `
    ...
    <textarea id="diagnosticLog" readonly spellcheck="false" style="width:100%;min-height:360px;resize:vertical;border:1px solid #dfe4ee;border-radius:10px;padding:12px;font:12px/1.45 ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;background:#fbfcff;color:#20283a"></textarea>`;
...
function cloneState(state) {
  return { n: state.n, assigned: Uint8Array.from(state.assigned), values: Int16Array.from(state.values), assignedCount: state.assignedCount };
}
...
// Teacher exists only to generate imitation data; none of this formula is present in the learned forward pass.
...
function makeValueTrainingState(n, targetVar, rng) {
  // Mix full repair-like and partial construction states so the value mechanism is not tied to one phase.
  if (rng() < 0.55) return makeRepairTrainingState(n, rng).state;
  const state = emptyState(n);
  const ids = Array.from({ length: n }, (_, i) => i).filter(i => i !== targetVar);
  for (let k = ids.length - 1; k > 0; k--) {
    const j = Math.floor(rng() * (k + 1)); [ids[k], ids[j]] = [ids[j], ids[k]];
  }
...
function buildValueCandidateSet(positiveValue, rng) {
  ...
  while (values.length < VALUE_NEGATIVES + 1) {
    let v;
    if (rng() < 0.65) {
      const radius = 1 + Math.floor(rng() * 48);
      v = positiveValue + (rng() < 0.5 ? -radius : radius);
    } else {
      v = DOMAIN_MIN + Math.floor(rng() * (DOMAIN_SPAN + 1));
    }
    v = clampValue(v);
    if (!seen.has(v)) { seen.add(v); values.push(v); }
  }
  for (let i = values.length - 1; i > 0; i--) {
    const j = Math.floor(rng() * (i + 1)); [values[i], values[j]] = [values[j], values[i]];
  }
  return { values: Int32Array.from(values), positiveIndex: values.indexOf(positiveValue) };
}

function makeTrainingSample(n, rng) {
  const repair = makeRepairTrainingState(n, rng);
  const ids = Array.from(repair.candidateIds);
  const teacherVar = ids[Math.floor(rng() * ids.length)];
  const valueState = makeValueTrainingState(n, teacherVar, rng);
  ...
  return {
    ...
    candidateIds: repair.candidateIds,
    valueState,
    teacherVar,
    teacherValue,
    valueCandidates: candidates.values,
    positiveIndex: candidates.positiveIndex
  };
}
...
function trainableParams() {
  ...
  return [
    ...
    params.Wproposal1, params.bproposal1, params.WproposalMean, params.bproposalMean, params.WproposalLogStd, params.bproposalLogStd,
    params.Wvalue1, params.bvalue1, params.Wvalue2, params.bvalue2
  ];
}
...
function nodeFeatureData(state) {
  ...
  for (let i = 0; i < state.n; i++) {
    ...
    data[p + 6] = state.assigned[i] ? (state.values[i] - DOMAIN_MIN) / DOMAIN_SPAN : 0;
  }
...
function contextVectorFromRep(rep, variableId) {
  const local0 = rep.h0.slice([variableId, 0], [1, EMBED_DIM]);
  const local = rep.h.slice([variableId, 0], [1, EMBED_DIM]);
  const global = rep.h.mean(0).reshape([1, EMBED_DIM]);
  return tf.concat([local0, local, global], 1);
}
function proposalTensorsFromRep(rep, variableId) {
  const x = contextVectorFromRep(rep, variableId);
  const hidden = tf.relu(tf.matMul(x, params.Wproposal1).add(params.bproposal1));
  const meanNorm = tf.sigmoid(tf.matMul(hidden, params.WproposalMean).add(params.bproposalMean)).reshape([]);
  const logStdNorm = tf.matMul(hidden, params.WproposalLogStd).add(params.bproposalLogStd).reshape([]).clipByValue(-7.5, -0.15);
  return { meanNorm, logStdNorm };
}
function candidateFeatureData(state, variableId, candidateValues) {
  ...
  for (let k = 0; k < candidateValues.length; k++) {
    ...
    out[3 * k] = (v - DOMAIN_MIN) / DOMAIN_SPAN;
    out[3 * k + 1] = assigned ? (v - current) / DOMAIN_SPAN : 0;
...
function proposalSnapshot(state, variableId) {
  return tf.tidy(() => {
    const rep = graphEmbeddings(state);
    const p = proposalTensorsFromRep(rep, variableId);
    const meanNorm = p.meanNorm.dataSync()[0];
    const logStdNorm = p.logStdNorm.dataSync()[0];
    return {
      mean: DOMAIN_MIN + DOMAIN_SPAN * meanNorm,
      sigma: Math.max(0.25, DOMAIN_SPAN * Math.exp(logStdNorm)),
      meanNorm, logStdNorm
    };
  });
}
...
function variableLossFromLogits(logits, candidateIds, n) {
  ...
  const posBce = tf.softplus(logits.neg()).mul(labels).sum().div(Math.max(1, posCount));
  const negBce = tf.softplus(logits).mul(negatives).sum().div(Math.max(1, negCount));
  const bce = posCount && negCount ? posBce.add(negBce).mul(0.5) : (posCount ? posBce : negBce);
  const posMargin = tf.relu(tf.scalar(VARIABLE_MARGIN).sub(logits)).mul(labels).sum().div(Math.max(1, posCount));
  const negMargin = tf.relu(tf.scalar(VARIABLE_MARGIN).add(logits)).mul(negatives).sum().div(Math.max(1, negCount));
...
function proposalNllFromRep(rep, variableId, teacherValue) {
  const p = proposalTensorsFromRep(rep, variableId);
  const targetNorm = tf.scalar((teacherValue - DOMAIN_MIN) / DOMAIN_SPAN);
  const sigmaNorm = tf.exp(p.logStdNorm);
  const z = targetNorm.sub(p.meanNorm).div(sigmaNorm);
  return z.square().mul(0.5).add(p.logStdNorm);
}
function trainingSampleLoss(sample) {
  const repairRep = graphEmbeddings(sample.repairState);
  const varLogits = variableLogitsFromRep(repairRep, sample.repairState);
  const variableLoss = variableLossFromLogits(varLogits, sample.candidateIds, sample.repairState.n);

  const valueRep = graphEmbeddings(sample.valueState);
  const proposalLoss = proposalNllFromRep(valueRep, sample.teacherVar, sample.teacherValue);
  const valueLogits = valueLogitsFromRep(valueRep, sample.valueState, sample.teacherVar, sample.valueCandidates);
  const logPolicy = tf.logSoftmax(valueLogits);
  const valueLoss = tf.gather(logPolicy, tf.tensor1d([sample.positiveIndex], 'int32')).mean().neg();

  return variableLoss
    .add(proposalLoss.mul(PROPOSAL_LOSS_WEIGHT))
    .add(valueLoss.mul(VALUE_SCORE_LOSS_WEIGHT));
}
...
function chooseRepairVariable(state, rng, stochastic = true) {
  ...
  const predicted = [];
  for (let i = 0; i < state.n; i++) if (sigmoidScalar(logits[i]) >= VARIABLE_THRESHOLD) predicted.push(i);
  if (predicted.length) {
    if (stochastic) return predicted[Math.floor(rng() * predicted.length)];
    return predicted.reduce((best, i) => logits[i] > logits[best] ? i : best, predicted[0]);
  }
...
function generateValueCandidates(state, variableId, rng, stochastic = true) {
  ...
  // Deterministic proposal coverage around the learned mean.
  for (const z of [-2.5, -2, -1.5, -1, -0.5, 0.5, 1, 1.5, 2, 2.5]) addCandidate(set, proposal.mean + z * proposal.sigma);

  if (stochastic) {
    for (let k = 0; k < PROPOSAL_SAMPLES; k++) addCandidate(set, gaussian(proposal.mean, proposal.sigma, rng));
  }

  // Generic exploration candidates; no problem-specific value formula is used.
  addCandidate(set, DOMAIN_MIN);
  addCandidate(set, DOMAIN_MAX);
  if (state.assigned[variableId]) {
    const current = state.values[variableId];
    addCandidate(set, current);
    for (const d of [1, 2, 4, 8, 16, 32]) { addCandidate(set, current - d); addCandidate(set, current + d); }
  }
...
function chooseValue(state, variableId, rng, stochastic = true, oracleValue = false, proposalOnly = false) {
  ...
  const generated = generateValueCandidates(state, variableId, rng, stochastic);
  const candidates = generated.values;
  const logits = valueSnapshot(state, variableId, candidates);
  if (!stochastic) {
...
function runLearned(n, rng, stochastic = true, oracleVariable = false, oracleValue = false, proposalOnly = false) {
  ...
  while (state.assignedCount < n && moves < maxMoves) {
    ...
    const v = chooseValue(state, i, rng, stochastic, oracleValue, proposalOnly);
...
function runSA(n, evaluations, rng) {
  ...
  for (let step = 0; step < evaluations && bestE > 0; step++) {
    const i = Math.floor(rng() * n), old = x[i]; x[i] = DOMAIN_MIN + Math.floor(rng() * (DOMAIN_SPAN + 1));
...
function endpointDiagnostics(n, rng, states = 24) {
  ...
  for (let s = 0; s < states; s++) {
    ...
    for (let i = 0; i < n; i++) {
      const pred = sigmoidScalar(logits[i]) >= VARIABLE_THRESHOLD;
...
function valueDiagnostics(n, rng, states = 10) {
  let proposalMae = 0, sigma = 0, scorerMae = 0, candidateGap = 0;
  for (let s = 0; s < states; s++) {
    const base = makeRepairTrainingState(n, rng), ids = Array.from(base.candidateIds);
    const i = ids[Math.floor(rng() * ids.length)];
    const target = teacherMeanValue(i);
    const prop = proposalSnapshot(base.state, i);
    proposalMae += Math.abs(prop.mean - target);
    sigma += prop.sigma;
    const generated = generateValueCandidates(base.state, i, rng, false);
    let minGap = Infinity;
    for (const v of generated.values) minGap = Math.min(minGap, Math.abs(v - target));
    candidateGap += minGap;
    const logits = valueSnapshot(base.state, i, generated.values);
    let best = 0; for (let k = 1; k < logits.length; k++) if (logits[k] > logits[best]) best = k;
    scorerMae += Math.abs(generated.values[best] - target);
  }
  return {
    proposalMae: proposalMae / states,
    sigma: sigma / states,
    scorerMae: scorerMae / states,
    candidateGap: candidateGap / states
  };
}
...
async function trainGeneric() {
  ...
  logLine('GNN_PROPOSAL_SCORER_DIAGNOSTIC v1');
  logLine(`CONFIG backend=${tf.getBackend()} train=${startN}..${maxN} states=${totalStates} rounds=${MESSAGE_ROUNDS} embed=${EMBED_DIM} proposalSamples=${PROPOSAL_SAMPLES} randomCandidates=${RANDOM_CANDIDATES} valueNegatives=${VALUE_NEGATIVES} budget=${MOVE_MULTIPLIER}n`);

  const rng = mulberry32(20260818); let completed = 0, updates = 0, lastLoss = NaN;
  while (completed < totalStates) {
    ...
    for (let b = 0; b < count; b++) {
      ...
      batch.push(makeTrainingSample(n, rng)); completed++;
    }
    lastLoss = await trainingUpdate(batch); updates++;
    if (updates % 4 === 0 || completed >= totalStates) {
      const ed = endpointDiagnostics(maxN, mulberry32(810000 + updates), 20);
      const vd = valueDiagnostics(maxN, mulberry32(820000 + updates), 8);
      const solve = solveRate(runLearned, maxN, 830000 + updates, 4);
      statusEl.textContent = `States ${completed}/${totalStates} · endpoint F1 ${(100*ed.f1).toFixed(0)}% · proposal MAE ${vd.proposalMae.toFixed(1)} σ ${vd.sigma.toFixed(1)} · scorer MAE ${vd.scorerMae.toFixed(1)} · solve@${maxN} ${(100*solve).toFixed(0)}%`;
      logLine(`TRAIN states=${completed} update=${updates} loss=${lastLoss.toFixed(4)} P=${(100*ed.precision).toFixed(1)}% R=${(100*ed.recall).toFixed(1)}% F1=${(100*ed.f1).toFixed(1)}% proposalMAE=${vd.proposalMae.toFixed(2)} proposalSigma=${vd.sigma.toFixed(2)} candidateGap=${vd.candidateGap.toFixed(2)} scorerMAE=${vd.scorerMae.toFixed(2)} solve@n${maxN}=${(100*solve).toFixed(0)}%`);
      await tf.nextFrame();
    }
  }

  trained = true; trainBtn.disabled = false; benchmarkBtn.disabled = false;
  statusEl.textContent = 'Proposal + scorer training complete. Running benchmark…';
  await runBenchmark();
}
...
async function runBenchmark() {
  ...
  const rows = [], rng = mulberry32(20260819), trials = 6;
  for (const n of benchmarkLengths()) {
    statusEl.textContent = `Benchmarking proposal + scorer GNN at n=${n}…`;
    let pSolved=0,hSolved=0,saSolved=0,proposalSolved=0,oracleVarSolved=0,oracleValueSolved=0,pViol=0,hViol=0;
    const pMoves=[], hMoves=[];
    for (let t=0;t<trials;t++) {
      const p=runLearned(n,rng,true,false,false,false); if(p.solved){pSolved++;pMoves.push(p.moves);} pViol+=violatedConstraintCount(p.x);
      if(runLearned(n,rng,true,false,false,true).solved) proposalSolved++;
      if(runLearned(n,rng,true,true,false,false).solved) oracleVarSolved++;
      if(runLearned(n,rng,true,false,true,false).solved) oracleValueSolved++;
  ...
  $('meanMae').textContent=vd.scorerMae.toFixed(1);
  ...
  $('metricLength').textContent=`n=${n} · GNN variable head + Gaussian proposal + Q(s,i,v) scorer · train ${trainedRange.min}…${trainedRange.max} · depth ${MESSAGE_ROUNDS}`;

  appendProbe(trainedRange.max); appendTrace(trainedRange.max); logLine('END');
  statusEl.textContent='Done. Gaussian proposes; Q(s,i,v) makes the final value decision.';
...
function appendProbe(n) {
  ...
  const proposal=proposalSnapshot(sample.state,i);
  const generated=generateValueCandidates(sample.state,i,mulberry32(9310+n),false);
  const vLogits=valueSnapshot(sample.state,i,generated.values);
  const ids=Array.from({length:vLogits.length},(_,k)=>k), vProbs=softmaxDistribution(vLogits,ids,VALUE_TEMPERATURE).sort((a,b)=>b[1]-a[1]);
  logLine(`PROBE_VALUE n=${n} variable=${i} teacherMean=${teacherMeanValue(i)} proposalMean=${proposal.mean.toFixed(2)} proposalSigma=${proposal.sigma.toFixed(2)} candidates=${generated.values.length} top=[${vProbs.slice(0,10).map(([k,p])=>`${generated.values[k]}:${(100*p).toFixed(2)}%`).join(',')}]`);
}

function appendTrace(n) {
  const rng=mulberry32(9400+n), state=emptyState(n), maxMoves=MOVE_MULTIPLIER*n; let moves=0;
  while(state.assignedCount<n&&moves<maxMoves){
    const rem=[];for(let i=0;i<n;i++)if(!state.assigned[i])rem.push(i);
    const i=rem[Math.floor(rng()*rem.length)], proposal=proposalSnapshot(state,i), v=chooseValue(state,i,rng,true,false,false);
    applyAction(state,i,v); moves++;
    if (moves <= 3) logLine(`TRACE_CONSTRUCT move=${moves} i=${i} proposalMean=${proposal.mean.toFixed(2)} sigma=${proposal.sigma.toFixed(2)} v=${v}`);
  }
  ...
  for(let step=0;step<16&&moves<maxMoves&&!isStrictChain(state.values);step++){
    const valid=new Set(violatedEndpointIds(state.values)),before=violatedConstraintCount(state.values),i=chooseRepairVariable(state,rng,true),proposal=proposalSnapshot(state,i),v=chooseValue(state,i,rng,true,false,false);
    applyAction(state,i,v);moves++;
    logLine(`TRACE_REPAIR step=${step} choose=${i} valid=${valid.has(i)?1:0} proposalMean=${proposal.mean.toFixed(2)} sigma=${proposal.sigma.toFixed(2)} v=${v} viol=${before}->${violatedConstraintCount(state.values)} energy=${violationEnergy(state.values)}`);
  }
...
statusEl.textContent=`TensorFlow.js ready · backend: ${tf.getBackend()}. Generic GNN action model: variable scorer + Gaussian proposal + candidate-value scorer.`;

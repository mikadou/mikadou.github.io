...
// Typed directed edges preserve smaller/greater operand roles in both directions.
...
function temperatureAt(stageEpisodes) {
  const t = Math.min(1, stageEpisodes / 120);
  return 1.35 - 0.55 * t;
}
...
async function trainRL() {
  ...
  for (let ep = 0; ep < total; ep++) {
    ...
    if (ep % 10 === 0 || ep === total - 1) {
      greedySolved = rollout(currentN, rng, false).solved;
    }
    ...
    if (ep % 10 === 0 || ep === total - 1 || readyToPromote) {
      statusEl.textContent = `Episode ${ep + 1}/${total} · curriculum n=${currentN}${currentN < maxN ? `/${maxN}` : ' (max)'} · terminal success ${(100 * rate).toFixed(0)}% · greedy ${greedySolved ? 'SOLVED' : 'not yet'} · T=${temp.toFixed(2)} · promotions ${promotions}${Number.isFinite(lastLoss) ? ` · loss ${lastLoss.toFixed(4)}` : ''}`;
      await tf.nextFrame();
    }
  }
...
async function runBenchmark() {
  ...
  for (const n of benchmarkLengths()) {
    statusEl.textContent = `Benchmarking terminal-reward GNN at n=${n}…`;
    let policySolved = 0;
    let saSolved = 0;
  ...
  $('metricLength').textContent = `n=${n} · domain 0…${DOMAIN_MAX} · terminal reward only`;
  statusEl.textContent = `Done. The GNN gets exactly ${n} joint (i,v) choices; feasibility is checked only after the complete assignment.`;
...
if (window.tf) {
  tf.ready().then(() => {
    statusEl.textContent = `TensorFlow.js ready · backend: ${tf.getBackend()}. Train the terminal-reward GNN curriculum to begin.`;
  });
} else {

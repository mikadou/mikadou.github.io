// Select a TensorFlow.js backend before the PPO engine is evaluated.
...
async function boot() {
  ...
  try {
    ...
    if (statusEl) statusEl.textContent = `TensorFlow.js ready · backend: ${backend}${platformNote}. Loading PPO GNN…`;
    await import('./app-gnn-ppo.js?v=20260810-20');
  } catch (err) {
    console.error(err);

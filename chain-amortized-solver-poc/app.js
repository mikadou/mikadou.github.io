...
// Customer-like recurring distribution: a descending global trend plus smooth motifs,
// regime shifts, and noise. The oscillations create many monotonicity violations.
function generateInstance(n, rng = Math.random) {
  ...
  const top = 112 + (rng() - .5) * 8;
  const bottom = 15 + (rng() - .5) * 8;
  const shockAt = Math.floor(n * (.25 + .5 * rng()));
  const shock = (rng() - .5) * 24;
  ...
  for (let i = 0; i < n; i++) {
    ...
    const localShock = i >= shockAt ? shock * Math.exp(-(i - shockAt) / Math.max(3, n * .18)) : 0;
    const noise = (rng() - .5) * 7;
...
// O(n * VALUE_COUNT) exact DP using suffix minima.
function solveExact(target) {
  ...
  for (let i = 1; i < n; i++) {
    ...
    curr.fill(INF);
    for (let v = 0; v < VALUE_COUNT; v++) {
      const u = v + 1; // previous value must be strictly larger
      if (u >= VALUE_COUNT || suffixCost[u] >= INF / 2) continue;
      const d = v - target[i];
      curr[v] = d * d + suffixCost[u];
      parent[i * VALUE_COUNT + v] = suffixArg[u];
    }
    [prev, curr] = [curr, prev];
  }

  let bestCost = INF, bestV = -1;
  for (let v = 0; v < VALUE_COUNT; v++) {
...
function greedyInitial(target) {
  ...
  let upper = MAX_VALUE;
  for (let i = 0; i < n; i++) {
    const lower = n - 1 - i; // leaves enough values for the suffix
    const hi = upper;
    x[i] = Math.round(clamp(target[i], lower, hi));
    upper = x[i] - 1;
  }
  return x;
}
...
function solveSA(target, evaluations = target.length * 180, rng = Math.random) {
  ...
  for (let step = 0; step < evaluations; step++) {
    ...
    if (rng() < .18) {
      ...
      proposal = clamp(old + (rng() < .5 ? -1 : 1) * (1 + Math.floor(rng() * span)), lo, hi);
    }
...
function buildModel() {
  ...
  const out = tf.layers.conv1d({ filters: VALUE_COUNT, kernelSize: 1, padding: 'same', activation: 'softmax' }).apply(z);
  const m = tf.model({ inputs: input, outputs: out });
  m.compile({ optimizer: tf.train.adam(0.002), loss: 'sparseCategoricalCrossentropy', metrics: ['accuracy'] });
  return m;
}
...
async function trainPolicy() {
  ...
  for (let b = 0; b < count; b++) {
    const t = generateInstance(n, rng);
    const exact = solveExact(t);
    instances.push(t);
  ...
  await model.fit(xs, ys, {
    ...
    callbacks: {
      onEpochEnd: async (epoch, logs) => {
        const loss = logs.loss?.toFixed(3) ?? '—';
        const acc = ((logs.acc ?? logs.accuracy ?? 0) * 100).toFixed(1);
        const val = ((logs.val_acc ?? logs.val_accuracy ?? 0) * 100).toFixed(1);
        statusEl.textContent = `Epoch ${epoch + 1}/${ep} · loss ${loss} · train ${acc}% · validation ${val}%`;
        await tf.nextFrame();
      }
    }
  });
...
function solvePolicy(target) {
  ...
  const n = target.length;
  const x = new Int32Array(n);
  let prev = VALUE_COUNT;
  for (let i = 0; i < n; i++) {
    const lo = n - 1 - i;
    const hi = Math.min(MAX_VALUE, prev - 1);
    let bestV = lo, bestP = -1;
...
function setupCanvas(canvas) {
  ...
  canvas.height = Math.floor(Math.max(260, rect.width * .46) * dpr);
...
function drawAxes(ctx, w, h, pad, xLabel, yLabel) {
  ...
  ctx.strokeStyle = '#dbe1eb'; ctx.lineWidth = 1;
  ...
  ctx.textAlign = 'right'; ctx.fillText(xLabel, w - 12, h - 10); ctx.textAlign = 'left';
}
...
function line(ctx, pts, color, width = 2.4) {
  ...
  ctx.stroke();
  ctx.fillStyle = color;
  for (const p of pts) { ctx.beginPath(); ctx.arc(p[0], p[1], 2.8, 0, Math.PI * 2); ctx.fill(); }
}
...
function drawScaling(rows) {
  ...
  const minN = Math.min(...rows.map(r => r.n)), maxN = Math.max(...rows.map(r => r.n));
  ...
  for (const r of rows) { const x = X(r.n); ctx.fillText(String(r.n), x - 6, h - pad + 17); }
  for (let k = 0; k <= 4; k++) { const yv = maxY * k / 4; const y = Y(yv); ctx.fillText(fmt(yv), 4, y + 4); }
  line(ctx, rows.map(r => [X(r.n), Y(r.policy)]), '#5b67d6');
  line(ctx, rows.map(r => [X(r.n), Y(r.sa)]), '#dd6b55');
}
...
function drawInstance(target, exact, policy, sa) {
  ...
  line(ctx, Array.from(target, (v, i) => [X(i), Y(v)]), '#9aa4b8', 1.5);
  line(ctx, Array.from(exact, (v, i) => [X(i), Y(v)]), '#2e8b72', 2.5);
  line(ctx, Array.from(policy, (v, i) => [X(i), Y(v)]), '#5b67d6', 2.2);
  line(ctx, Array.from(sa, (v, i) => [X(i), Y(v)]), '#dd6b55', 2.0);
}
...
window.addEventListener('resize', () => {
  // Charts are redrawn on the next benchmark; avoid retaining large benchmark data globally.
});
